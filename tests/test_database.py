...
def test_customer_contact_three_day_followup_rapport_and_sold_archive(tmp_path: Path):
    ...
    db.move_customer_to_inspection(customer_id,"2026-08-05")
    assert db.customer_contacts()==[] and db.customer_contacts(stage="inspection")[0]["id"]==customer_id
    assert db.customer_contacts(stage="inspection")[0]["inspection_date"]=="2026-08-05"
    db.return_customer_to_callers(customer_id)
    assert db.customer_contacts()[0]["id"]==customer_id and db.customer_contacts()[0]["inspection_date"] is None
