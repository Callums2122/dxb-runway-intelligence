...
class VehicleDialog(QDialog):
    def __init__(self, db: Database, parent=None, source_customer=None, vehicle=None):
        ...
        self.stock_number=QLineEdit(); self.stock_number.setPlaceholderText("Optional · e.g. 13833")
    ...
    def values(self)->dict:
        return {"vehicle_name":self.name.text().strip(),"market_model_year":self.market_year.value() or None,"market_trim":self.market_trim.text().strip(),"external_stock_number":self.stock_number.text().strip(),"mileage_km":self.mileage.value() or None,"purchase_type":self.purchase_type.currentData(),"purchase_price_aed":self.purchase.value(),"expected_sale_price_aed":self.expected.value(),"purchased_date":self.purchased.date().toString("yyyy-MM-dd"),"notes":self.notes.text().strip()}
