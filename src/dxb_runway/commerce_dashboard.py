...
class CommerceDashboardPage(Page):
    def __init__(self,db:Database):
        ...
        acquired=QFrame();acquired.setObjectName("commerceCard");acquired_layout=QVBoxLayout(acquired);acquired_layout.setContentsMargins(17,14,17,15);acquired_layout.addWidget(self.section("Cars brought into stock"));acquired_sub=QLabel("Every vehicle added to Runway, including cash purchases and consignments");acquired_sub.setObjectName("commerceMuted");acquired_layout.addWidget(acquired_sub);acquired_grid=QGridLayout();acquired_grid.setContentsMargins(0,7,0,0);acquired_grid.setHorizontalSpacing(12);self.acquired={}
        for column,(key,label) in enumerate((("30","PAST 30 DAYS"),("90","PAST 90 DAYS"),("all","ALL TIME"))):
            stat=QFrame();stat.setObjectName("commerceStat");stat_layout=QVBoxLayout(stat);stat_layout.setContentsMargins(14,11,14,11);name=QLabel(label);name.setObjectName("commerceLabel");value=QLabel("—");value.setObjectName("commerceStatValue");detail=QLabel("vehicles stocked");detail.setObjectName("commerceMuted");stat_layout.addWidget(name);stat_layout.addWidget(value);stat_layout.addWidget(detail);acquired_grid.addWidget(stat,0,column);self.acquired[key]=value
        acquired_layout.addLayout(acquired_grid);self.body.addWidget(acquired)
    ...
    def refresh(self):
        ...
        for vehicle in vehicles:
            try:stocked_dates.append(date.fromisoformat(str(vehicle["purchased_date"])[:10]))
            except (TypeError,ValueError):pass
        self.acquired["30"].setText(str(sum(1 for stocked in stocked_dates if today-timedelta(days=29)<=stocked<=today)))
        self.acquired["90"].setText(str(sum(1 for stocked in stocked_dates if today-timedelta(days=89)<=stocked<=today)))
        self.acquired["all"].setText(str(sum(1 for stocked in stocked_dates if stocked<=today)))
