...
class VehicleDeskPage(Page):
    def __init__(self, db: Database):
        ...
        self.tier_table=QTableWidget(12,7); self.tier_table.setHorizontalHeaderLabels(["MONTH","BUDGET · AED","BASELINE RATE","TIER 3 · FACTORY TARGET","TIER 2 · FACTORY TARGET","TIER 1 · FACTORY TARGET","ACHIEVED TIER / RATE"]); self.tier_table.setWordWrap(True); self.tier_table.setEditTriggers(QAbstractItemView.EditTrigger.NoEditTriggers); self.tier_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows); self.tier_table.verticalHeader().hide(); self.tier_table.horizontalHeader().setStretchLastSection(True); self.tier_table.setMinimumHeight(470); matrix_layout.addWidget(self.tier_table); root.addWidget(tier_matrix)
    ...
    def refresh_tier_table(self,salary:Decimal,table_budget:Decimal|None=None)->None:
        ...
        for row_index in range(12):
            ...
            values=[self.month.itemText(row_index),f"{table_budget:,.0f}","4%",f"AED {target_profits[0]:,.0f}\nFactory {float(original_t3*100):g}% · Pay 5%",f"AED {target_profits[1]:,.0f}\nFactory {float(original_t2*100):g}% · Pay 6.5%",f"AED {target_profits[2]:,.0f}\nFactory {float(original_t1*100):g}% · Pay 8%",f"{result.tier.value} / {float(result.rate*100):g}% · {kpi_hits} KPI"]
            for column,value in enumerate(values):
                item=table_item(str(value),Qt.AlignmentFlag.AlignVCenter|Qt.AlignmentFlag.AlignRight if column else Qt.AlignmentFlag.AlignVCenter,COLORS["green"] if column==6 and result.tier!=CommissionTier.BASELINE else COLORS["cyan"] if column==6 else None)
                if column in {3,4,5}:
                    item.setToolTip("Factory target before any KPI reduction. The first line is the AED profit required; the second line is the original target percentage and commission pay rate.")
                self.tier_table.setItem(row_index,column,item)
            self.tier_table.setRowHeight(row_index,62)
        for column,width in enumerate([145,140,115,215,215,215]): self.tier_table.setColumnWidth(column,width)
