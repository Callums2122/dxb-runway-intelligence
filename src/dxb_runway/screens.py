...
class StockLevelPage(Page):
    def __init__(self, db: Database):
        ...
        for column,(key,label,color) in enumerate([("tier3","T3 spend guide",COLORS["cyan"]),("tier2","T2 spend guide",COLORS["purple"]),("tier1","T1 spend guide",COLORS["green"])]):
            card=MetricCard(label,accent=color); self.spend_targets[key]=card; spend_grid.addWidget(card,0,column)
    ...
    def refresh(self)->None:
        ...
        for key,label,target in [("tier3","T3",spend_targets[0]),("tier2","T2",spend_targets[1]),("tier1","T1",spend_targets[2])]:
            ...
            if cash_margin>0:
                spend_needed=money(target_profit/cash_margin); spend_percent=(spend_needed/live_budget*Decimal("100")) if live_budget>0 else Decimal("0"); spend_color=COLORS["green"] if spend_percent<=Decimal("95") else COLORS["amber"] if spend_percent<=Decimal("100") else COLORS["red"]; detail=f"{spend_percent:.1f}% of budget · target AED {target_profit:,.0f} · {adjusted_target*100:g}% after KPI · {cash_margin*100:.1f}% margin"; self.spend_targets[key].set_value(f"AED {spend_needed:,.0f}",detail,spend_color)
            else:
