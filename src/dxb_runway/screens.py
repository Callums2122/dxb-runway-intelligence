...
class VehicleDeskPage(Page):
    ...
    def refresh_tier_table(self,salary:Decimal)->None:
        ...
        for row_index in range(12):
            ...
            target_profits=[money(budget*target) for target in (t3,t2,t1)]
            values=[self.month.itemText(row_index),f"{budget:,.0f}","4%",f"AED {target_profits[0]:,.0f}\n{float(t3*100):g}% / 5%",f"AED {target_profits[1]:,.0f}\n{float(t2*100):g}% / 6.5%",f"AED {target_profits[2]:,.0f}\n{float(t1*100):g}% / 8%",f"{result.tier.value} / {float(result.rate*100):g}% · {kpi_hits} KPI"]
