...
class VehicleDeskPage(Page):
    ...
    def refresh(self)->None:
        ...
        for key,label,target,commission_rate in [("tier3","Tier 3",t3,Decimal("0.05")),("tier2","Tier 2",t2,Decimal("0.065")),("tier1","Tier 1",t1,Decimal("0.08"))]:
            target_profit=money(budget*target); commission=money(target_profit*commission_rate); total=money(salary+commission); total_aed,total_gbp=dual_amount(total,rate); self.tier_earnings[key].set_value(total_aed,f"{total_gbp} · {label} goal {target*100:g}% after KPI · rate {commission_rate*100:g}% · commission AED {commission:,.0f} + salary AED {salary:,.0f}")
