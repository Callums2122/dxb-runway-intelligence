...
class MainWindow(QMainWindow):
    def __init__(self, db: Database, icon_path: Path | None = None):
        ...
        self.pages={"dashboard":DashboardPage(db),"todo":TodayTodoPage(db),"success":SuccessChecklistPage(db),"kpi":KPITrackerPage(db),"contacts":CustomerContactPage(db),"inspection":InspectionPage(db),"templates":WhatsAppTemplatesPage(db),"stock":StockLevelPage(db),"stock_action":StockActionPlanPage(db),"vehicles":VehicleDeskPage(db),"gym_today":GymDashboardPage(db),"gym_training":GymTrainingPage(db),"gym_nutrition":GymNutritionPage(db),"gym_progress":GymProgressPage(db),"gym_meals":GymMealsPage(db),"transactions":TransactionsPage(db),"debt":DebtPage(db),"scenarios":ScenarioPage(db),"budgets":BudgetsPage(db),"calendar":CalendarPage(db),"schedule":SchedulePage(db),"pipeline":PipelinePage(db),"goals":GoalsPage(db),"vehicle_history":VehicleHistoryPage(db),"reports":ReportsPage(db),"intelligence":IntelligencePage(db),"ask_runway":AskRunwayPage(db),"settings":SettingsPage(db)}
