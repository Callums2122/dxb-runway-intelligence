...
from .growth_tools import ProfitRescuePage, TierOneSimulatorPage
...
NAV_SECTIONS = [
    ...
    ("leads", "LEADS", COLORS["purple"], [("stock", "🚘", "Stock level"), ("profit_rescue", "🛟", "Profit rescue"), ("tier_simulator", "🏁", "Tier 1 simulator"), ("vehicles", "💼", "Vehicle desk"), ("kpi", "📊", "KPI tracker"), ("schedule", "🗓", "Schedule"), ("stock_action", "⚡", "Stock action plan"), ("vehicle_history", "📈", "Vehicle performance"), ("pipeline", "🟢", "Appointments"), ("todo", "📝", "Today's to-do"), ("success", "🎯", "Checklist to success"), ("calendar", "📅", "Calendar")]),
...
class MainWindow(QMainWindow):
    def __init__(self, db: Database, icon_path: Path | None = None):
        ...
        self.pages={"dashboard":DashboardPage(db),"todo":TodayTodoPage(db),"success":SuccessChecklistPage(db),"kpi":KPITrackerPage(db),"contacts":CustomerContactPage(db),"inspection":InspectionPage(db),"templates":WhatsAppTemplatesPage(db),"stock":StockLevelPage(db),"profit_rescue":ProfitRescuePage(db),"tier_simulator":TierOneSimulatorPage(db),"stock_action":StockActionPlanPage(db),"vehicles":VehicleDeskPage(db),"gym_today":GymDashboardPage(db),"gym_training":GymTrainingPage(db),"gym_nutrition":GymNutritionPage(db),"gym_progress":GymProgressPage(db),"gym_meals":GymMealsPage(db),"transactions":TransactionsPage(db),"debt":DebtPage(db),"scenarios":ScenarioPage(db),"budgets":BudgetsPage(db),"calendar":CalendarPage(db),"schedule":SchedulePage(db),"pipeline":PipelinePage(db),"goals":GoalsPage(db),"vehicle_history":VehicleHistoryPage(db),"reports":ReportsPage(db),"intelligence":IntelligencePage(db),"ask_runway":AskRunwayPage(db),"settings":SettingsPage(db)}
    ...
    def set_invoice_status(self,text:str,okay:bool)->None:
        self.sync_status.setText(text); self.sync_status.setStyleSheet(f"color:{COLORS['green'] if okay else COLORS['amber']};font-size:10px")

    def _invoice_data_changed(self)->None:
        self.refresh_all(); self.mobile_sync.schedule()
    ...
    def open_palette(self)->None:
        commands=[("Open buying intelligence","nav:intelligence"),("Open Ask Runway","nav:ask_runway"),("Open appointments","nav:pipeline"),("Open profit rescue","nav:profit_rescue"),("Open Tier 1 simulator","nav:tier_simulator"),("Open today's to-do list","nav:todo"),("Open checklist to success","nav:success"),("Open KPI tracker","nav:kpi"),("Open customer contact","nav:contacts"),("Open inspection","nav:inspection"),("Open WhatsApp templates","nav:templates"),("Open stock level","nav:stock"),("Open stock action plan","nav:stock_action"),("Open vehicle desk","nav:vehicles"),("Open vehicle performance","nav:vehicle_history"),("Open calendar","nav:calendar"),("Open schedule","nav:schedule"),("Open settings","nav:settings"),("Refresh all data","refresh")]
