...
from .screens import (
    ...
    ScenarioPage, SettingsPage, StockLevelPage, TodayTodoPage, TransactionsPage, VehicleDeskPage, VehicleHistoryPage,
...
NAV_SECTIONS = [
    ("leads", "LEADS", COLORS["purple"], [("todo", "✓", "Today's to-do"), ("kpi", "◫", "KPI tracker"), ("contacts", "◉", "Customer contact"), ("inspection", "⌕", "Inspection"), ("templates", "✉", "WhatsApp templates"), ("stock", "▦", "Stock level"), ("vehicles", "◈", "Vehicle desk"), ("calendar", "▣", "Calendar"), ("scenarios", "⌁", "Scenario lab")]),
    ...
    ("other", "MISC / OTHER", COLORS["amber"], [("goals", "◎", "Momentum"), ("vehicle_history", "◷", "Vehicle performance"), ("reports", "▥", "Reports"), ("settings", "⚙", "Settings")]),
]
...
class MainWindow(QMainWindow):
    def __init__(self, db: Database, icon_path: Path | None = None):
        ...
        add_nav(*OVERVIEW_NAV,"overview"); nav_side.addSpacing(7)
        ...
        right=QVBoxLayout(); right.setContentsMargins(0,0,0,0); right.setSpacing(0); top=QFrame(); top.setObjectName("topbar"); tl=QHBoxLayout(top); tl.setContentsMargins(20,10,20,10); self.context=QLabel("OVERVIEW"); self.context.setObjectName("eyebrow"); tl.addWidget(self.context); tl.addStretch(); self.sync_status=QLabel("Phone sync checking…"); self.sync_status.setObjectName("muted"); tl.addWidget(self.sync_status); command=QPushButton(f"⌕  Search or command     {COMMAND_LABEL} K"); command.clicked.connect(self.open_palette); tl.addWidget(command); quick=QPushButton("＋"); quick.setToolTip(f"Quick add transaction · {COMMAND_LABEL}+N"); quick.setProperty("primary",True); quick.clicked.connect(self.quick_add); tl.addWidget(quick); right.addWidget(top)
        self.stack=QStackedWidget(); right.addWidget(self.stack,1); shell.addLayout(right,1)
        self.pages={"dashboard":DashboardPage(db),"todo":TodayTodoPage(db),"kpi":KPITrackerPage(db),"contacts":CustomerContactPage(db),"inspection":InspectionPage(db),"templates":WhatsAppTemplatesPage(db),"stock":StockLevelPage(db),"vehicles":VehicleDeskPage(db),"gym_today":GymDashboardPage(db),"gym_training":GymTrainingPage(db),"gym_nutrition":GymNutritionPage(db),"gym_progress":GymProgressPage(db),"gym_meals":GymMealsPage(db),"transactions":TransactionsPage(db),"debt":DebtPage(db),"scenarios":ScenarioPage(db),"budgets":BudgetsPage(db),"calendar":CalendarPage(db),"goals":GoalsPage(db),"vehicle_history":VehicleHistoryPage(db),"reports":ReportsPage(db),"settings":SettingsPage(db)}
    ...
    def open_palette(self)->None:
        commands=[("Go to overview","nav:dashboard"),("Open today's to-do list","nav:todo"),("Open KPI tracker","nav:kpi"),("Open customer contact","nav:contacts"),("Open inspection","nav:inspection"),("Open WhatsApp templates","nav:templates"),("Open stock level","nav:stock"),("Open vehicle desk","nav:vehicles"),("Open gym today","nav:gym_today"),("Log a gym workout","nav:gym_training"),("Open nutrition tracker","nav:gym_nutrition"),("Open gym progress","nav:gym_progress"),("Open meals and bowls","nav:gym_meals"),("Open vehicle performance","nav:vehicle_history"),("Go to transactions","nav:transactions"),("Go to debt control","nav:debt"),("Open scenario lab","nav:scenarios"),("Open budgets","nav:budgets"),("Open financial calendar","nav:calendar"),("Open reports","nav:reports"),("Open settings","nav:settings"),("Add transaction","add"),("Refresh all data","refresh")]
