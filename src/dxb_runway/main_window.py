...
from .screens import (
    ...
    ProjectFivePage, ScenarioPage, SettingsPage, StockLevelPage, TodayTodoPage, TransactionsPage, VehicleDeskPage, VehicleHistoryPage
)
...
NAV_SECTIONS = [
    ("leads", "LEADS", COLORS["purple"], [("todo", "✓", "Today's to-do"), ("kpi", "◫", "KPI tracker"), ("contacts", "◉", "Customer contact"), ("inspection", "⌕", "Inspection"), ("templates", "5%", "Project 5%"), ("stock", "▦", "Stock level"), ("vehicles", "◈", "Vehicle desk"), ("calendar", "▣", "Calendar"), ("scenarios", "⌁", "Scenario lab")]),
...
class MainWindow(QMainWindow):
    def __init__(self, db: Database, icon_path: Path | None = None):
        ...
        self.pages={"dashboard":DashboardPage(db),"todo":TodayTodoPage(db),"kpi":KPITrackerPage(db),"contacts":CustomerContactPage(db),"inspection":InspectionPage(db),"templates":ProjectFivePage(db),"stock":StockLevelPage(db),"vehicles":VehicleDeskPage(db),"transactions":TransactionsPage(db),"debt":DebtPage(db),"scenarios":ScenarioPage(db),"budgets":BudgetsPage(db),"calendar":CalendarPage(db),"goals":GoalsPage(db),"vehicle_history":VehicleHistoryPage(db),"reports":ReportsPage(db),"settings":SettingsPage(db)}
    ...
    def whatsapp_imported(self,import_id:int)->None:
        self.refresh_all(); rows=self.db.query("SELECT chat_name,status FROM whatsapp_imports WHERE id=?",(import_id,))
        if not rows:return
        linked="PROFILE UPDATED" if rows[0]["status"]=="imported" else "NEEDS REVIEW"
        self.context.setText(f"PROJECT 5% · {rows[0]['chat_name'].upper()} · {linked}")
        QTimer.singleShot(6000,lambda:self.context.setText(next(label.upper() for key,_,label in NAVIGATION if self.pages[key] is self.stack.currentWidget())))
