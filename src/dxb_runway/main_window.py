...
NAV_SECTIONS = [
    ("leads", "LEADS", COLORS["purple"], [("stock", "▦", "Stock level"), ("vehicles", "◈", "Vehicle desk"), ("calendar", "▣", "Calendar"), ("scenarios", "⌁", "Scenario lab")]),
...
class MainWindow(QMainWindow):
    def __init__(self, db: Database, icon_path: Path | None = None):
        ...
        self.pages={"dashboard":DashboardPage(db),"stock":StockLevelPage(db),"vehicles":VehicleDeskPage(db),"transactions":TransactionsPage(db),"debt":DebtPage(db),"scenarios":ScenarioPage(db),"budgets":BudgetsPage(db),"calendar":CalendarPage(db),"goals":GoalsPage(db),"vehicle_history":VehicleHistoryPage(db),"reports":ReportsPage(db),"settings":SettingsPage(db)}
