...
from .screens import (
    ...
    ScenarioPage, SettingsPage, StockLevelPage, TransactionsPage, VehicleDeskPage
)
...
class MainWindow(QMainWindow):
    def __init__(self, db: Database, icon_path: Path | None = None):
        ...
        self.pages={"dashboard":DashboardPage(db),"stock":StockLevelPage(db),"vehicles":VehicleDeskPage(db),"transactions":TransactionsPage(db),"debt":DebtPage(db),"scenarios":ScenarioPage(db),"budgets":BudgetsPage(db),"calendar":CalendarPage(db),"goals":GoalsPage(db),"reports":ReportsPage(db),"settings":SettingsPage(db)}
